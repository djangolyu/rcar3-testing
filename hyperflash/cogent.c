...
static void rpc_hf_mode_man(struct rpc_info *info)
{
	...
	rpc_setl(info, RPC_PHYCNT,
		 ...
		 RPC_PHYCNT_CAL | RPC_PHYCNT_MEM(3));
	rpc_setl(info, RPC_PHYCNT, 0xffffffff, 0x80038263);
...
struct device *rpc_hf_dev; /* RPC-IF HyperFlash Memory device */

static int check_ssl_delay(struct rpc_info *info)
{
	u32 value;

	value = ioread32(info->rpc_base + RPC_SSLDR);
	if (value != 0x400) {
		pr_debug("Setting up SSL delay.\n");
		iowrite32(0x400, info->rpc_base + RPC_SSLDR);
	}
	iowrite32(0x400, info->rpc_base + RPC_SSLDR);
	value = ioread32(info->rpc_base + RPC_SSLDR);
	pr_debug("SSLDR = 0x%08x\n", value);

	pr_debug("    PHYINT = 0x%08x\n", ioread32(info->rpc_base + RPC_PHYINT));
	pr_debug("PHYOFFSET1 = 0x%08x\n", ioread32(info->rpc_base + 0x80));
	pr_debug("PHYOFFSET2 = 0x%08x\n", ioread32(info->rpc_base + 0x84));
	return 0;
}

static void __dump_mem(struct rpc_info *info)
{
	rpc_hf_mode_ext(info);

	pr_debug("MEM: 0x%08x\n", ioread32(info->flash_base));
	pr_debug("MEM: 0x%08x\n", ioread32(info->flash_base + 0x4));
	pr_debug("MEM: 0x%08x\n", ioread32(info->flash_base + 0x400000));
}
...
static int rpc_flash_init(void)
{
	...
	dev = root_device_register("rpcifhyperflash");
	if (IS_ERR(dev)) {
	...
	check_ssl_delay(info);
	__dump_mem(info);

#if 0
	retval = rpc_hf_sector_erase(info, 0x0);
	pr_info("Erase sector retval = %d\n", retval);
	rpc_hf_mode_ext(info);
#endif
	return 0;
...
static void rpc_flash_exit(void)
{
	...
	release_mem_region(info->rpc_res->start,
			   resource_size(info->rpc_res));
	kfree(info);
	root_device_unregister(rpc_hf_dev);
}
